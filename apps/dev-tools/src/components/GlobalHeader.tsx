...
export function GlobalHeader() {
    ...
    return (
        <>
            <header className="fixed top-0 left-0 right-0 z-[100] bg-[#030508]/90 backdrop-blur-md border-b border-white/5 pointer-events-auto h-16 flex items-center justify-between px-6 w-full">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button
                        ...
                        className={`flex items-center justify-center w-9 h-9 rounded-full border transition-all shadow-lg ${isHelperOpen ? 'bg-teal-500/20 border-teal-500/50 text-teal-400' : 'bg-[#1e1e1e]/60 border-white/5 text-gray-400 hover:text-white hover:bg-white/5'}`}
                        title="Dev-Tools Helper"
                    >
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <HelperModal open={isHelperOpen} onClose={() => setIsHelperOpen(false)} />
        </>
    );
}
