...
fn load_json<T: for<'de> Deserialize<'de>>(path: &PathBuf, label: &str) -> Result<T, String> {
    ...
    serde_json::from_str(&raw)
        .map_err(|e| format!("parse {label} at {}: {e}", path.display()))
}
...
pub fn load_content_bundle() -> Result<ContentBundle, String> {
    ...
    let occupations: Vec<Occupation> = load_json(&data_path("occupations.json")?, "occupations.json")?;
    let talent_trees: Vec<RawTalentTree> =
        load_json(&data_path("talentTrees.json")?, "talentTrees.json")?;

    Ok(ContentBundle {
        traits: traits.into_iter().map(|entry| (entry.id.clone(), entry)).collect(),
        skills: skills.into_iter().map(|entry| (entry.id.clone(), entry)).collect(),
        items: items.into_iter().map(|entry| (entry.id.clone(), entry)).collect(),
...
pub fn load_character(character_id: &str) -> Result<RawCharacter, String> {
    ...
    load_json(&path, &format!("character {character_id}"))
}
