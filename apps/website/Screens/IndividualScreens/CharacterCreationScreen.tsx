...
import { Player, Stats, Trait } from '../../types';
import { ALL_TRAITS } from '../../mockData';
import { generateCharacterPortrait, enhanceAppearancePrompt } from '../../game-master/worldarchitect/story';
  ...
  const TraitItem: React.FC<{ trait: Trait; active?: boolean }> = ({ trait, active }) => (
    <Tooltip content={trait.description}>
      <button 
        ...
        className={`w-full p-2 text-left border rounded-sm transition-all group flex items-center justify-between ${
          active 
          ...
          <div className="flex gap-1">
             {(['IDENTITY', 'TRAITS', 'STATS'] as CreationTab[]).map(tab => (
               <button
                 ...
                 className={`px-3 py-1 text-[9px] mono uppercase font-bold tracking-widest border-b-2 transition-all ${
                   activeTab === tab 
