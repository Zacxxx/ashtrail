...
export interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
    variant?: 'primary' | 'secondary' | 'danger' | 'ghost' | 'accent' | 'glass';
    isLoading?: boolean;
    size?: 'sm' | 'md' | 'lg';
}
...
export const Button: React.FC<ButtonProps> = ({
    ...
    const variants = {
        ...
        glass: "bg-white/10 backdrop-blur-3xl hover:bg-white/20 text-white shadow-[0_8px_32px_rgba(0,0,0,0.3)] transition-all duration-700"
    };
...
export const CardHeader: React.FC<{ children: React.ReactNode; className?: string }> = ({
    ...
    <div className={`bg-zinc-800/50 px-4 py-2 border-b border-zinc-800 text-[10px] font-bold uppercase tracking-widest text-zinc-400 flex justify-between items-center ${className}`}>
        {children}
    </div>
);
...
export const CardContent: React.FC<{ children: React.ReactNode; className?: string }> = ({
    ...
    <div className={`p-4 flex-1 overflow-hidden flex flex-col ${className}`}>
        {children}
    </div>
);
